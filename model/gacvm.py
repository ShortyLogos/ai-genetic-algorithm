...
class Domains:
    ...
    def __init__(self, ranges, names):
        ...
        if ranges.dtype not in (float, np.float16, np.float32, np.float64):  # , np.float128):
            raise ValueError(
                'Invalid input parameters in Domains : ranges must be a 2D numpy ndarray class of any float.')
        if ranges.ndim != 2:
        ...
        if ranges.shape[0] <= 0 or ranges.shape[1] != 2:
            raise ValueError(
                'Invalid input parameters in Domains : ranges must be a 2D numpy ndarray class with n x 2 describing n x 1D intervals.')
        if np.any(ranges[:, 0] > ranges[:, 1]):
            raise ValueError(
                'Invalid input parameters in Domains : all range must describe a 1D interval with [min, max] such as min <= max.')
...
class RouletteWheelSelectionStrategy(SelectionStrategy):
    ...
    def select(self, genitor, fitness_data, selection_rate, selection_size):
        def select_each(rnd_val, fitness_data):
            return fitness_data[
                np.argmax(fitness_data[fitness_data['cumul'] <= max(fitness_data[0]['cumul'], rnd_val)]['cumul'])][
                'index']
...
class Parameters:
    ...
    def __init__(self, selection_strategy=RouletteWheelSelectionStrategy(),
                 crossover_strategy=WeightedAverageCrossoverStrategy(), mutation_strategy=GeneMutationStrategy()):
        if not isinstance(selection_strategy, SelectionStrategy):
            raise ValueError(
                'Invalid input parameters in Parameters : selection_strategy must be an SelectionStrategy object.')
        if not isinstance(crossover_strategy, CrossoverStrategy):
            raise ValueError(
                'Invalid input parameters in Parameters : crossover_strategy must be an CrossoverStrategy object.')
        if not isinstance(mutation_strategy, MutationStrategy):
            raise ValueError(
                'Invalid input parameters in Parameters : mutation_strategy must be an MutationStrategy object.')
...
class History:
    ...
    def _log_history(self, best_solution, best_fitness, worst_fitness, average_fitness, std_dev_fitness,
                     median_fitness):
        self._last_epoch += 1
    ...
    def gradient(self, average_size=5):
        if self._last_epoch < average_size + 1:
            return None
        return np.average(
            self._fitness_history[self._last_epoch - average_size:self._last_epoch, 0] - self._fitness_history[
                                                                                         self._last_epoch - average_size - 1:self._last_epoch - 1,
                                                                                         0])
...
class GeneticAlgorithm:
    ...
    def __init__(self, problem_definition=None, parameters=Parameters()):
        if not isinstance(problem_definition, ProblemDefinition) and problem_definition is not None:
            raise ValueError(
                'Invalid input value in GeneticAlgorithm.problem_definition property : value must be a ProblemDefinition object.')
        if not isinstance(parameters, Parameters):
            raise ValueError(
                'Invalid input value in GeneticAlgorithm.parameters property : value must be a Parameters object.')
    ...
    def _setup(self):
        ...
        self._current_epoch = 0
        self._population_1 = np.empty((self._parameters.population_size, self._problem_definition.dimension),
                                      dtype=np.float64)
        self._population_2 = np.empty((self._parameters.population_size, self._problem_definition.dimension),
                                      dtype=np.float64)
    ...
    def _process_fitness(self):
        ...
        if np.any(self._genitors_fit['value'] < 0.):
            raise ValueError(
                'Invalid fitness. Negative value generated by fitness function. All fitness value must be positive. Suggestion : adjust the fitness function such as all return values are greather or equal than zero for the specified domain.')
        if np.sum(self._genitors_fit['value']) <= self._parameters._epsilon_min_fitness:
            raise ValueError(
                'Invalid fitness. All values are too close to zero. Suggestion : adjust the fitness function such as it returns some values greather than zero for the specified domain.')
    ...
    def _process_elitism(self):
        if self._parameters.elitism_size:
            self._offsprings[0:self._parameters.elitism_size] = self._genitors[
                self._genitors_fit[0:self._parameters.elitism_size]['index']]

    def _breed(self):
        g1 = self._parameters.selection_strategy.select(self._genitors, self._genitors_fit,
                                                        self._parameters.selection_rate,
                                                        self._parameters.population_size - self._parameters.elitism_size)
        g2 = self._parameters.selection_strategy.select(self._genitors, self._genitors_fit,
                                                        self._parameters.selection_rate,
                                                        self._parameters.population_size - self._parameters.elitism_size)
        self._parameters.crossover_strategy.breed(g1, g2, self._offsprings[self._parameters.elitism_size:])

    def _mutate(self):
        self._parameters.mutation_strategy.mutate(self._offsprings[self._parameters.elitism_size:],
                                                  self._parameters.mutation_rate, self._problem_definition.domains)
